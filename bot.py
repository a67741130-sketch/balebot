...
# ================= JOIN FIX =================
def join_game(user_id, code):
    g = get_game(code)

    if not g:
        send(user_id, "❌ چنین کدی وجود ندارد")
        return

    if g["p2"] is not None:
        send(user_id, "❌ این بازی قبلاً پر شده")
        return

    g["p2"] = user_id
    update(g)

    send(g["p1"], "🎮 بازیکن وارد بازی شد!")
    send(user_id, "🎮 به بازی وصل شدی!")
...
def webhook():
    ...
    if "message" in data:
        ...
        if chat_id in waiting_join:
            code = text.strip()
            del waiting_join[chat_id]
            join_game(chat_id, code)
            return "ok"
    ...
    if "callback_query" in data:
        ...
        if (chat_id == g["p1"] and g["p1_move"]) or (chat_id == g["p2"] and g["p2_move"]):
            return "ok"
        ...
        with game_lock:
            ...
            if g["p1_move"] and (g["p2"] or g["mode"] == "bot"):
                if g["mode"] == "bot":
                    g["p2_move"] = bot_move()
